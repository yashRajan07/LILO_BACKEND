...
async def handle_esp32_connection(websocket: WebSocket):
    ...
    if is_quiet_hours_active():
        logger.info("Bedtime lock is active — rejecting ESP32 connection")
        await websocket.send_text(json.dumps({
            "type": "error",
            "reason": "bedtime_lock",
            "message": "It's bedtime! LILO is sleeping now. Come back tomorrow morning!",
        }))
        await websocket.close(code=4003, reason="Bedtime lock active")
        return
    ...
    try:
        # Extract client IP for device status tracking
        if websocket.client:
        ...
        set_device_online(ip_address=client_ip)
        ...
        # Track session duration for screen-time analytics
        session_duration = time.monotonic() - session_start
        try:
            log_session_duration(session_duration)
            logger.info(f"Session duration logged: {session_duration:.1f}s")
        except Exception as e:
            logger.error(f"Failed to log session duration: {e}")
